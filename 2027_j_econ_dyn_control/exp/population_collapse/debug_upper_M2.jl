...
Plots:
  1) The partition of the (G0, L0) plane into Omega_1 ... Omega_6  (CairoMakie, static)
  2) The 3D region M2 = { (G0,L0,A0) : A0 < min(U_j, A0star_j) on Omega_j }  (GLMakie, interactive)
  3) A fixed-G0 slice of M2 in the (L0, A0) plane (CairoMakie, static)

Structured so each Omega_j has its own small set of functions (region formula +
classifier check), and a `for j in 1:6` loop calls them one at a time -- comment
out entries in `regions_to_run` below to isolate a single region while debugging.
=#
...
# ---------------------------------------------------------------------------
using CairoMakie
function plot_omega_partition(; G0max=5.0, L0max=20.0, n=600, savepath="omega_partition.png"#="omega_partition.pdf"=#)
...
# per region, using GLMakie for interactivity (rotate/zoom).
# Regions are plotted ONE AT A TIME via region_A0max, then combined.
# ---------------------------------------------------------------------------
using GLMakie
function plot_region_3d(j::Int; G0max=5.0, L0max=20.0, n=150)
    ...
    ax = Axis3(fig[1,1], xlabel="G0", ylabel="L0", zlabel="A0",
               title="M2^($j) boundary over Omega_$j")
    surface!(ax, G0s, L0s, A0, colormap=:viridis, nan_color=:transparent)
    display(fig)
    return fig, G0s, L0s, A0
end

using GLMakie
function plot_M2_3d(; G0max=5.0, L0max=20.0, n=150, regions_to_run=1:6)
    G0s = collect(range(1e-6, G0max, length=n))
    L0s = collect(range(1e-6, L0max, length=n))
...
# ---------------------------------------------------------------------------
using CairoMakie
function plot_M2_slice(G0::Float64; L0max=20.0, n=2000, savepath="M2_slice_G0_$(G0).png")
    L0s = collect(range(1e-6, L0max, length=n))
...
# ---------------------------------------------------------------------------
# Driver: run regions one at a time so you can pause/inspect between them
# ---------------------------------------------------------------------------
function main(; G0max=5.0, L0max=20.0, regions_to_run=1:6, slice_G0=1.0)
    ...
    for j in regions_to_run
        println("Plotting M2^($j) individually -- inspect, then press Enter to continue...")
        plot_region_3d(j; G0max=G0max, L0max=L0max)
