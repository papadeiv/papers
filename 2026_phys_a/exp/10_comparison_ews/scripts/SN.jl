function SN()
        ...
        μ_min, μ_max = μ0, μf 
        threshold = maximum(get_equilibria(f, μf, domain=[-10,10]).unstable)
        for (solution_index, solution) in enumerate(ensemble.state)
                ...
                printstyled("SN test case: progressing trajectory $solution_index of $(convert(Integer, Ne))\r"; bold=true, underline=true, color=:light_magenta)
                ...
                writeout(ews, "ews/sn/$(400+solution_index).csv")
        end

        # Compute the number of steps in the filtered subseries
        ews = readin("ews/sn/1.csv")
        ...
        lines!(ax1L, μ_filtered, ensemble_mean_variance, color = (:red,0.75), linewidth = 3.0)
        lines!(ax2L, μ_filtered, ensemble_mean_ews, color = (:red,0.75), linewidth = 3.0)

        # Plot the tipping point and format the axes
        lines!(ax1L, [μ_max, μ_max], [-2,2], color = :black, linewidth = 3.0, linestyle = :dash)
        lines!(ax2L, [μ_max, μ_max], [-2,2], color = :black, linewidth = 3.0, linestyle = :dash)
