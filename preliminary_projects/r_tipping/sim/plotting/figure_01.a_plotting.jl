...
# Loop over only the critical rates
@showprogress for n in 1:Nεc
        # Get the index of the critical rate
        local idx = convert(Int64, Ic[n])

        # Import the data from csv
        local solution = readin("../data/figure_01/solutions/$idx.csv")
        local t = solution[:,1]
        local μ = solution[:,2]
        local u = solution[:,3]

        # Plot the state's solution 
        lines!(ax1, μ, u, linewidth = 1, color = idx, colormap = :thermal, colorrange = (1,Nε))
        # Plot the parameter's shift
        lines!(ax2, t, μ, linewidth = 1, color = idx, colormap = :thermal, colorrange = (1,Nε))
end
...
# Compute the color indices of the critical rates
lower = convert(Int64, floor((Ic[1]/Nε)*256))
upper = convert(Int64, floor((Ic[end]/Nε)*256))

# Plot the colorbar for the critical rates only
Colorbar(fig,
         alignmode = Outside(),
         bbox = BBox(929,1200,520,900), # Order is: left, right, bottom, up
         size = 45,
         height = 148,
         spinewidth = 5.0,
         limits = (1, 256),
         colormap = Makie.to_colormap(:thermal)[lower:upper],
         ticksvisible = false,
