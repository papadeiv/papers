...
file = matread("../data/cont_nu_step_320/branch.mat")
key = file["branch"]
λ = key[:,2]
ν = key[:,3]
u_2 = key[:,5]
...
for n in 1:N_bif
        ...
        if idx_stb[n] < 0
                ...
                lines!(ax, ν[idx_bif[n]:idx_bif[n+1]], u_2[idx_bif[n]:idx_bif[n+1]], linewidth = 3.0, color = :magenta4)
        else
                # Plot the branch of unstable solutions
                lines!(ax, ν[idx_bif[n]:idx_bif[n+1]], u_2[idx_bif[n]:idx_bif[n+1]], linewidth = 3.0, color = :magenta4#=, linestyle = :dash=#)
        end
end
...
ν_plt = Vector{Float64}(undef, 2*N_plt)
idx_plt = [1,171,221,301,351,length(ν)] 

# Loop over the plotting indices
for n in 1:(2*N_plt)
        # Read in solution from formatted index
        filename = @sprintf("../data/cont_nu_step_320/solution_%07d.mat", idx_plt[n]-1)
...
scatter!(ax, ν_plt, u_2[idx_plt], markersize = 30, strokewidth = 5, strokecolor = :black, color = :lightslateblue)

# Array of boolean variables for plotting labels and ticks
plot_bool = [true, false, false]
...
for n in 1:N_plt
        ...
        # Plot the real field
        lines!(ax, x, u_re[:,n+N_plt], linewidth = 3.0, color = :blue)
        # Plot the imaginary field
        lines!(ax, x, u_im[:,n+N_plt], linewidth = 3.0, color = :red)
end
...
# Export the figure 
save("../fig/fig:homotopy_continuation_step_320.png", fig)
