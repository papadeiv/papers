...
function main()
        ...
        @showprogress for n in 1:Ne
                ...
                ax1 = Axis(fig[1,1:2], xlabel = L"\textbf{years}", ylabel = L"\textbf{flow (250-500m)}", limits = (time[1], time[end], -27,-22))
                ax2 = Axis(fig[2,1:2], xlabel = L"\textbf{years}", ylabel = L"\textbf{early-warning signal}", limits = (time[1], time[end], 0.0, 1.1))
                ax3 = Axis(fig[1,3], xlabel = L"\textbf{residuals}", ylabel = L"\textbf{density}", limits = (-0.125, 0.125, nothing, 15))
                ax4 = Axis(fig[2,3], xlabel = L"\mathbf{V(x)}", ylabel = L"\mathbf{x}", limits = (-1,1,-1,1))
                ...
                poly!(ax1, Point2f[(tw[1], -27), 
                                   (tw[end], -27),
                                   (tw[end], -22),
                                   (tw[1], -22),
                                  ], color = (CtpGray, 0.1), strokecolor = :black, strokewidth = 1.0)
                lines!(ax1, [time[idx], time[idx]], [-27, -22], color = :red, linestyle = :dash, linewidth = 2.0)
                lines!(ax2, t[(Nt-1):(Nt+n-2)], ews[1:n], color = :red, linewidth = 4.0)
                lines!(ax2, [time[idx], time[idx]], [-27, -22], color = :red, linestyle = :dash, linewidth = 2.0)
