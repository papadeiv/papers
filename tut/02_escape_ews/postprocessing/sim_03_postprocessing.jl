...
@showprogress for n in 1:Ns
        ...
        if tipped
                ...
                writeout([n, (n + tip_idx - 1), t_t[tip_idx], μ_t[tip_idx]], "../data/sim_03/tipping_point.csv")
                break
        end
        ...
        # Export the data 
        writeout(hcat(t_t, μ_t, trend), "../data/sim_03/trend/$n.csv")
        writeout(residuals, "../data/sim_03/residuals/$n.csv")
        writeout(hcat(bins, pdf), "../data/sim_03/histograms/$n.csv")
end

# Export the solutions of the non-linear least-squares problem
writeout(coefficients, "../data/sim_03/coefficients.csv")

# Export the EWSs
writeout(hcat(μ_var, u_var, u_esc, true_esc), "../data/sim_03/ews.csv")

# Export the horizontal and vertical shifts
writeout(hcat(x_s, y_s), "../data/sim_03/shifts.csv")
